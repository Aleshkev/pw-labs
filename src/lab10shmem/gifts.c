...
// popular names
const char *elfs_names[MAX_ELFS] = {"Mirek",  "Zuzia", "Gienia",
                                    "Macius", "Ela",   "Stasia"};
const char *reindeers_names[MAX_REINDEERS] = {
    "Janek", "Zosia", "Franek", "Jozek", "Asia", "Olek", "Ruda"};
const char *gifts[MAX_GIFTS] = {"dolls", "legos", "a train", "a whip",
                                "a bike"};
...
void deliver(int i) { usleep((rand() % 4) * NAP_MICROSECS); }
...
int main(void) {
  ...
  struct SharedStorage *s = shared_storage;
  s->begin = 0;
  s->end = 0;
  sem_init(&s->mutex, true, 1);
  sem_init(&s->sem_free_slots, true, BUFFSIZE);
  sem_init(&s->sem_filled_slots, true, 0);
  s->n_gifts_to_deliver = LOOPS * N_ELFS;
