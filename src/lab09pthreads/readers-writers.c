...
void before_write(struct ReadWriteLock* rw) {
  ASSERT_ZERO(pthread_mutex_lock(&rw->mutex));
  {
    ...
    // printf("wait %i %i %i %i\n", rw->n_reading, rw->n_writing,
    //        rw->n_readers_waiting, rw->n_writers_waiting);
    ...
    --rw->n_writers_waiting, ++rw->n_writing;
    ASSERT_ZERO(pthread_cond_signal(&rw->readers));
    ASSERT_ZERO(pthread_cond_signal(&rw->writers));
  }
...
void* reader_main(void* data) {
  ...
  while (library->working) {
    // fprintf(stderr, "Reader waiting to read: %d\n", library->book);
...
void* writer_main(void* data) {
  ...
  while (library->working) {
    int n = rand() % 10;
    // fprintf(stderr, "Writer waiting to write: %d\n", n);
    before_write(&library->rw_lock);
    fprintf(stderr, "Writer writes: %d\n", n);
    snprintf(library->book, sizeof(library->book),
             "six times a number %d %d %d %d %d %d", n, n, n, n, n, n);
