...
int child() {
  ...
  // Be annoying - replace SIGINT handler to just print a message and do
  // nothing.
...
int sigtimedwait_seconds(const sigset_t* set, siginfo_t* info, double seconds) {
  struct timespec timeout = {.tv_sec = (int)seconds,
...
int parent(pid_t child_pid) {
  ...
  // We won't wait for signals anymore, so we unblock default handlers, just in
  // case.
